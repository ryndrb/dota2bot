...
local tAllAbilityBuildList = {
                        ...
                        {3,1,1,2,3,6,1,2,2,2,6,3,3,3,6},--pos2
}
...
function X.ConsiderAvalanche()
	...
	do
		if J.CanCastOnNonMagicImmune(e) and e:IsChanneling() then
	...
	then
		...
		if J.IsValidTarget(target) and J.CanCastOnNonMagicImmune(target) and J.IsInRange(target, bot, nCastRange)
...
function X.ConsiderToss()
	...
	then
		if Avalanche:IsFullyCastable() then 
			...
			for i=1,#enemies do
				if J.IsValidTarget(enemies[i]) and J.CanCastOnNonMagicImmune(enemies[i]) and enemies[i]:GetHealth() < nDamage + nDamage2 then
			...
			if furthestTarget ~= nil and GetUnitToUnitDistance(furthestTarget, bot) > nRadius then
				...
				if J.IsValidTarget(tTarget) and tTarget:GetTeam() ~= bot:GetTeam() then
	...
	-- if J.IsInTeamFight(bot, 1300)  and J.CanCastOnNonMagicImmune(bot) == true
	-- then
	...
	if J.IsGoingOnSomeone(bot) and J.CanCastOnNonMagicImmune(bot) == true
	then
		local target = bot:GetTarget()
		if J.IsValidTarget(target) and J.CanCastOnNonMagicImmune(target)
		then
			if J.IsInRange(target, bot, nRadius) then
				 return BOT_ACTION_DESIRE_LOW, target
			elseif J.IsInRange(target, bot, nRadius) == false and J.IsInRange(target, bot, nCastRange) == true then
...
function X.ConsiderTreeGrab()
	if not TreeGrab:IsFullyCastable() or bot:HasModifier("modifier_tiny_tree_grab") == true
	then
		return BOT_ACTION_DESIRE_NONE, nil
	end
	
	local nCastRange = J.GetProperCastRange(false, bot, TreeGrab:GetCastRange())
	
	if J.IsRetreating(bot) == false and bot:GetHealth() > 0.15 * bot:GetMaxHealth() and bot:DistanceFromFountain() > 1000 then
...
function X.ConsiderTreeThrow()
	if not TreeThrow:IsFullyCastable() or bot:HasModifier("modifier_tiny_tree_grab") == false
	then
		return BOT_ACTION_DESIRE_NONE, nil;
	end
...
function X.ConsiderTreeVolley()
	if not TreeVolley:IsFullyCastable() or bot:HasScepter() == false 
	then
		return BOT_ACTION_DESIRE_NONE, nil;
	end
	...
	then
		local target = bot:GetTarget();
		if J.IsValidTarget(target) and J.CanCastOnNonMagicImmune(target) and J.IsInRange(target, bot, nCastRange) == true
		then
			local trees = bot:GetNearbyTrees(nRadius);
			if #trees >= 3 then
				return BOT_ACTION_DESIRE_HIGH, target:GetLocation();
