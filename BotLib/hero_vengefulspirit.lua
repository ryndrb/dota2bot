...
function X.ConsiderMagicMissile()
    ...
    for _, enemyHero in pairs(nEnemyHeroes) do
        ...
        then
            ...
            elseif enemyHero:IsChanneling() and fManaAfter > fManaThreshold1 + 0.15 then
                return BOT_ACTION_DESIRE_HIGH, enemyHero
            end
            ...
            if J.IsEarlyGame() then
                local nInRangeTower = enemyHero:GetNearbyTowers(250, true)

                if J.IsValidBuilding(nInRangeTower[1]) and nInRangeTower[1]:GetAttackTarget() == enemyHero then
                    return BOT_ACTION_DESIRE_HIGH, enemyHero
                end
            end
        end
...
function X.ConsiderWaveOfTerror()
    ...
    if J.IsPushing(bot) and fManaAfter > fManaThreshold1 and #nAllyHeroes <= 2 and #nEnemyHeroes == 0 and bAttacking then
        for _, creep in pairs(nEnemyCreeps) do
            if J.IsValid(creep) and J.CanBeAttacked(creep) and not creep:HasModifier('modifier_vengefulspirit_wave_of_terror') then
            ...
            for _, enemyHero in pairs(nEnemyHeroes) do
                ...
                then
                    ...
                    if  #nInRangeAlly >= #nInRangeEnemy and not (#nInRangeAlly > #nInRangeEnemy + 2) then
                        if targetScore < enemyHeroScore then
                            target = enemyHero
