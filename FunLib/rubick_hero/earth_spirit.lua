...
function X.ConsiderStolenSpell(ability)
	...
	local hasAbility, spell = J.HasAbility(bot, 'earth_spirit_stone_caller')

	if abilityName == 'earth_spirit_stone_caller'
	then
		StoneRemnant = ability
    ...
    -- if abilityName == 'earth_spirit_petrify'
    -- then
    ...
    if abilityName == 'earth_spirit_rolling_boulder'
    then
		RollingBoulder = ability
    ...
    if abilityName == 'earth_spirit_magnetize'
    then
		Magnetize = ability
    ...
    if abilityName == 'earth_spirit_boulder_smash'
    then
		BoulderSmash = ability
    ...
    if abilityName == 'earth_spirit_geomagnetic_grip'
    then
		GeomagneticGrip = ability
...
function X.ConsiderBoulderSmash()
	...
	local stoneNearby = X.IsStoneNearby(bot:GetLocation(), nAttackRange)
...
function X.ConsiderRollingBoulder()
	...
	do
		...
		then
			...
			if X.IsStoneInPath(loc, GetUnitToUnitDistance(bot, enemyHero))
			or nStone == 0
			then
	...
	then
		...
		then
			...
			if X.IsStoneInPath(loc, GetUnitToUnitDistance(bot, target))
			then
				return BOT_ACTION_DESIRE_HIGH, loc, false
			else
				return BOT_ACTION_DESIRE_HIGH, loc, true
...
function X.ConsiderGeomagneticGrip()
	...
	then
		...
		local isThereStoneNearTarget = X.IsStoneNearTarget(target)
...
-- HELPER FUNCS --
function X.IsStoneNearby(location, radius)
...
function X.IsStoneInPath(location, dist)
