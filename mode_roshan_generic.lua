...
function GetDesire()
    ...
    if shouldKillRoshan and initDPSFlag then
        ...
        local nRoshanDesire = Max(GetRoshanDesire(), 0) * mul
        ...
        if hasSameOrMoreHero or (not hasSameOrMoreHero and J.HasEnoughDPSForRoshan(aliveHeroesList)) then
            if (J.IsRoshanCloseToChangingSides() and not (J.IsRoshan(botTarget) and J.IsInRange(bot, botTarget, 900) and J.GetHP(botTarget) > 0.25)) then
                nRoshanDesire = nRoshanDesire - 0.25
            end

            if #nEnemyHeroes > 0 then
                nRoshanDesire = nRoshanDesire - (#nEnemyHeroes * (0.9 / 5))
            end
